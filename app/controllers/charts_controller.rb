...
#    @tags = current_user.tags.all.uniq.sort_by{|t| t.name.downcase}
    ...
    if not params[:tag].blank? #LOOK FOR SINGLE TAG TO FILTER NOTES ON
      ...
      @notes = Kaminari.paginate_array(@chart.notes.find(:all, :order => "date", :conditions => ['tags.id = ?', tag.id], :joins => [:tags])).page(params[:page]).per(6)
    elsif not params[:tags].blank? #LOOK FOR MULTIPLE TAGS TO FILTER NOTES ON
      filter_tags= Tag.find(:all, :conditions => {:id => params[:tags]})
      @notes = Kaminari.paginate_array(@chart.notes.find(:all, :joins => :tags, :order => "date", :conditions => {:tags => {:id => filter_tags}})).page(params[:page]).per(6)
    else #SHOW ALL NOTES
  ...
  def new
    ...
    unless current_user.tags.blank?
      @tags = current_user.tags.sort_by(&:name).collect{|t| t.name}.uniq
    else
      @tags = []
    end
